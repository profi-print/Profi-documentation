/*
 * Общее хранилище скриншотов заказов через IndexedDB.
 *
 * Зачем: localStorage ограничен ~5-10 МБ на весь сайт, а скриншоты в base64
 * очень быстро съедают этот лимит (ошибка "exceeded the quota" в любом другом
 * месте сайта — Палеты, Расходники и т.д. — как только localStorage забит).
 * IndexedDB не имеет такого маленького лимита (обычно сотни МБ - несколько ГБ),
 * поэтому сами картинки теперь хранятся там, а в localStorage у заказа остаётся
 * только маленький флаг has_screenshot: true.
 *
 * Подключать этот файл нужно ДО остального кода страницы:
